...
class Diagnostic
  ...
  # a Fixnum.
  def longest(list)
    list.max(2) { |a, b| a.length <=> b.length }
  end
  ...
  def transform(words)
    words.each_with_object(Hash.new()) do | e,m |
    m[:words] += e.split(' ').map(&:capitalize).join(' ')
    end
  end
  # Question 6
  ...
  def read_file(filename)
    wc = File.open(ARGV[filename]) do |file|
    file.each_with_object(Hash.new(0)) do |line, accumulator|
    accumulator[:words] += line.split(' ').Concatenate
  end
end
